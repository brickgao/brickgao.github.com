...
KMP 算是最常用的算法中效率还不错的单模式匹配算法了，具体来说算法分为两个部分，第一部分是在预处理阶段处理模式串，构建 next 数组，复杂度为 `O(m)`，第二部分就是匹配过程了，复杂度是 `O(m + n)`。
...
Bitmap 算法有一个很明显的缺点是存储状态时和模式串的长度以及字符编码长度相关，虽然可以用多个变量拼称一个更大的变量，但随之匹配时间和预处理模式串时间也辉持续增加。
