...
KMP 算是最常用的算法中效率还不错的单模式匹配算法了，具体来说算法分为两个本分，第一部分是在预处理阶段处理模式串，构建 next 数组，复杂度为 `O(m)`，第二部分就是匹配过程了，复杂度是 `O(n)`。
...
预处理所用的时间复杂度为 `O(m + sigma)`，其中 sigma 是字符集的大小。搜索阶段的平均复杂度为 `O(mn)`，最好情况下能够达到 `O(n / m)`，当模式串是非周期性的时候，最坏情况下需要进行 3n 次字符比较操作。
...
Bitmap 是一个基于二进制位表示字符存在的近似匹配算法，适用范围与字符串长度和变量大小相关，实践复杂度是 `O(m * n)`，在每个字符出现频率稀疏的情况下速度快。
...
AC 自动机的预处理复杂度位 `O(m)`，搜索复杂度位 `O(n)`。

AC 自动机的算法主要分为三个步骤：构造一个 Tire 树、构造失败指针和进行匹配。

构造 Tire 树就是构造一个前缀树。

构建失败指针的构造：若一个节点的字符为 ch，沿着他父节点的失败指针走，直到走到一个节点，他的子节点里面也有字符为 ch 的节点，然后就把当前节点的失败节点指向字符也为 ch 的那个儿子，如果走到根节点还没有找到，那么就把失败节点指向根节点。

匹配方式是对于 Tire 树上的一点，若当前字符匹配，则继续沿着该路径；若当前字符不匹配，走向其失败节点，当指针指向 root 时，该次匹配结束。
...
主要与 BM 算法的区别是构建了一个由所有模式串反转构成的 Tire 树，其他的匹配原则还是遵循好后缀和坏字符匹配。
